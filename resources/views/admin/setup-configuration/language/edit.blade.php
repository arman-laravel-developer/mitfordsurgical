    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h4 class="page-title">Language Edit</h4>
            </div>
        </div>
    </div>
                        ...
                        <div class="tab-pane show active" id="basic-form-preview">
                            <form action="{{route('language.update', ['id' => $language->id])}}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div>
                                        <input type="text" value="{{$language->name}}" class="form-control @error('name') is-invalid @enderror" name="name" id="inputEmail3" placeholder="Name"/>
                                        @error('name')
                                    ...
                                    <div>
                                        ...
                                        <select class="form-control" name="code" data-bs-toggle="select2">
                                            @if(File::exists($flagsPath))
                                                <option value="" disabled>Select Language Code</option>
                                                @foreach(File::files($flagsPath) as $path)
                                                    ...
                                                    @if(!in_array($fileName, $languagesArray))
                                                        <option value="{{ $fileName }}" data-image="{{ asset('admin/assets/images/flags/'.$fileName.'.png') }}"
                                                                @if($fileName == $language->code) selected @endif>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div>
                                        <input type="text" value="{{$language->app_lang_code}}" class="form-control @error('app_lang_code') is-invalid @enderror" name="app_lang_code" id="app_lang_code" placeholder="{{translate('Put ISO 639-1 code for your language')}}"/>
                                        @error('app_lang_code')
                                        <div class="alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
